...
```kotlin
// presentation/list/OrdersListContract.kt
...
`AsyncState<T>` is a standard sealed class this steering set assumes exists
(`Idle` / `Loading` / `Success(data)` / `Error(throwable)`, ~30 lines) —
define it once in your `:core` module if `Kolt/libs` isn't in the workspace.
**If it is,** `utils/src/commonMain/.../state/AsyncState.kt` is this exact
shape and is a vetted "reuse as-is" per [kolt-libs.md](kolt-libs.md) — use
it, don't redefine it. (It happens to live in a `commonMain` source set in
Kolt; that doesn't matter here, it compiles and runs fine as a plain
dependency of an Android-only module.)
...
(`UiStateEventsViewModel`, `UiStateEventsAndroidViewModel`,
`UiEventsViewModel`, `UiEventsAndroidViewModel`) even on an Android-only
project where the "breaks on iOS" objection doesn't apply — two reasons
remain: the `AndroidViewModel`-based variants go against official Android
guidance ("don't use `AndroidViewModel`" — inject `Application`/context via
DI instead of tying the base class to it), and the naming is confusing
(`UiEventType` is used for one-shot *effects*, which reads like it means
user-driven *events*). Use `androidx.lifecycle:lifecycle-viewmodel` directly
with a small shared base:
...
class OrdersListViewModel @Inject constructor(
    ...
    private val pagination = PaginationDelegate(viewModelScope) { page -> getOrdersUseCase(page) }
...
Navigation is Navigation 3 — see [navigation.md](navigation.md) for why and
for the `NavDisplay`/`entryProvider` setup. The composable named in that
`entryProvider` (call it `OrdersListRoute`, matching Now in Android's naming)
is where the ViewModel is instantiated and the only place that mutates the
back stack.

```kotlin
// presentation/list/OrdersListRoute.kt — instantiated from entryProvider in the app's NavDisplay
@Composable
...
anyway — it's the standard shape across the Android-MVI ecosystem (Orbit,
Circuit, etc.), and the loss-of-delivery risk Google warns about is
specifically config-change/process-death races, which
`flowWithLifecycle(lifecycle)` in the collector above (not a bare
`LaunchedEffect(Unit)` + `collect`) closes for the vast majority of cases.
...
- Every `Image`/`Icon` that conveys information has a `contentDescription`;
  every purely decorative one has `contentDescription = null` so TalkBack
  skips it — never leave it unset (TalkBack reads out the resource name/no-op
  garbage otherwise).
...
- Semantic grouping: related content that should be announced together
  ...
  or a single merged node, not N separate unlabeled nodes TalkBack reads out
  one at a time.
...
- [ ] ViewModel extends `MviViewModel`, is annotated `@HiltViewModel`, and lives in that screen's package.
- [ ] ViewModel is constructed only inside the Route composable named in `entryProvider` (via `hiltViewModel()`).
...
- [ ] Root composable has a `@Preview` (`androidx.compose.ui.tooling.preview.Preview`) with fake `State`.
