...
Extends `MviViewModel<State, Intent, Effect>` from `compose-utils` (KMP-ready — `androidx.lifecycle` ≥ 2.8.0 targets `commonMain`). `dispatch(intent)` is the single entry point. `onIntent` is the handler — it may be `suspend` and may call `updateState` and `sendEffect`.
...
fun CartRoute(vm: CartViewModel = viewModel(), nav: Navigator) {
    ...
    // collectEffects is a lifecycle-aware helper from compose-utils.
